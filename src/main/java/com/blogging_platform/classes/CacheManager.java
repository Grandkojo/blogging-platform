 ...
 * - Hashing: postByIdCache (ConcurrentHashMap) for O(1) lookup by id –
 * analogous to a DB hash index.
 * - Caching: publishedPostsCache holds the full list; search/sort run on this
 * in memory instead of querying the DB.
 * - Sorting: QuickSort for ordering results (relates to in-memory ordering like
 * indexed DB sort).
 */
@Component
public class CacheManager {
    ...
    /**
     * Hash index for O(1) lookup by post id – similar to a database primary-key
     * index.
     */
    private Map<String, PostRecord> postByIdCache = new ConcurrentHashMap<>();
    /**
     * Post id -> tag names for in-memory search by tag (populated at refresh if
     * tagService set).
     */
    ...
    /**
     * Returns paginated published posts, optionally filtered by search query and
     * sorted.
     * Uses the in-memory cache for fast response.
     *
     * @param page   0-based page number
     * @param size   items per page
     * @param query  optional search term (title, author, tags)
     * @param sortBy optional sort key ("date_desc", "date_asc", "title_asc", etc.)
     * @return paginated result
     */
    public PagedResult<PostRecord> getPaginatedPublishedPosts(
            int page,
            int size,
            String query,
            String sortBy) {
        ...
        try {
            ...
            // Log error and proceed with stale cache
            System.err.println("Cache refresh failed: " + e.getMessage());
        }

        // Get filtered list
        List<PostRecord> filtered = searchPosts(query);

        List<PostRecord> sorted = new ArrayList<>(filtered);
        sortPosts(sorted, sortBy);
        ...
        List<PostRecord> pageContent = (fromIndex < total)
    ...
    private void refreshIfNeeded() throws DatabaseException {
    long now = System.currentTimeMillis();
    if (now - lastRefresh > REFRESH_INTERVAL_MS || publishedPostsCache.isEmpty()) {
        refreshCache();
        lastRefresh = now;
    }
}
    ...
    /**
     * Loads all published posts into cache and builds the tag index for
     * search-by-tag.
     *
    ...
    public void refreshCache() throws DatabaseException {
        ...
        System.out.println("[CACHE] Refreshing from Database at: " + new java.util.Date());
    ...
    /**
     * Returns a copy of the cached published posts list, refreshing from the DB
     * first.
     *
    ...
    /**
     * In-memory search: filters cached posts by title, author, or tag
    ...
    private boolean matchesSearch(PostRecord p, String q) {
        if (p.title() != null && p.title().toLowerCase().contains(q))
            return true;
        if (p.author() != null && p.author().toLowerCase().contains(q))
            return true;
     ...
     * @param sortBy one of: "date_desc", "date_asc", "title_asc", "title_desc",
    ...
    private Comparator<PostRecord> comparatorFor(String sortBy) {
        if (sortBy == null)
            return (a, b) -> compareDateDesc(a.publishedDate(), b.publishedDate());
        switch (sortBy) {
            case "date_asc":
            ...
            case "title_asc":
                return Comparator.comparing(p -> p.title() != null ? p.title().toLowerCase() : "",
            ...
            case "author_asc":
                return Comparator.comparing(p -> p.author() != null ? p.author().toLowerCase() : "",
                        String.CASE_INSENSITIVE_ORDER);
            case "author_desc":
                return (a, b) -> String.CASE_INSENSITIVE_ORDER.compare(
    ...
    /**
     * Clears the cache and reloads from the DB. Call after comment/review create,
     * update, or delete.
     */
